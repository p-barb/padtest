...
from ltest.geometry.symmetric_solid import SymmetricSolidGeometry as Geometry
from ltest.model.model import Model

class SymmetricSolidModel(Model, Geometry):
    ...
    dynamic_boundary_condtions : dict, None
        ...
        default setting is adopted. Dictionary with keys `XMin`,
        `XMax`, `YMin` and `YMax`, with supported values
        'None' and 'Viscous'. By default None.
    ...
    load(filename)
        Loads saved test.
    failure_test(testid, test, max_load=np.inf, start_load=50, load_factor=2, load_increment=0, start_from='construction')
        Test the foundation until the model does not converge.
    load_test(testid, load, start_from='construction', delete_phases=True)
        Conducts a load test in the model.
    safety_test(testid, start_from, test='incremental', SumMsf=None, Msf=0.1)
    ...
    def __init__(self, s_i, g_i, g_o, b, d, b1, d1, soil, concrete,
        ...
        dynamic_boundary_condtions : dict, None
            ...
            'None' and 'Viscous'. By default None.
        """
        Geometry.__init__(self, b, d, b1, d1, dstrata=dstrata, wt=wt,
    ...
    def _build_load(self):
        ...
        self._g_i.set(self._load.Distribution, "Linear")
    ...
    def _set_output_precalc(self):
        ...
        for soil in self._g_i.Soils:
            if soil.Name.value == 'Soil_{:.0f}_1'.format(soil_idx):
                ...
                break
        for loc in self._output_location:
            self._output_point[loc] = self._g_o.addcurvepoint("node", soil_o, (loc * self._b / 2, -self._d))
    ...
    def _extract_phase_results(self, iphase, previphase, ophase, sumMstage, SumMsf, Fy, Uy):
        """Extracts results form the output of the initial phases calculation.
        ...
        if self._g_i.LineLoad_1_1.Active[iphase] is not None:
            if self._g_i.LineLoad_1_1.Active[iphase].value:
                ...
                Fy = qy * self._b1
        return sumMstage, SumMsf, Fy, qy, Uy, load_start, load_end
